...
export async function streamText(options) {
  const provider = providerFor(config.textProvider);
  if (provider.streamText) return provider.streamText(options);
  const result = await provider.generateText(options);
  if (result.text) options.onDelta?.(String(result.text));
  return result;
}
