...
import { generateText, streamText as streamProviderText } from './providers/index.js';
...
function notifyStream(listener, event) {
  if (typeof listener !== 'function') return;
  try {
    listener(event);
  } catch {
    // A disconnected browser must not interrupt the persisted Agent turn.
  }
}
...
export async function runPiAgent({
  ...
  function finishProviderCall(call, status, message = null, error = null) {
    ...
    if (message) {
      ...
      providerResult = {
        ...
        cache: {
          mode: 'provider_managed_runtime_stream',
  ...
  function nativeProviderStream(context, options) {
    const call = beginProviderCall(context, 'pi_native_provider_stream');
    let upstream;
    try {
  ...
  function adapterProviderStream(context, options) {
    const useStreamingAdapter = Boolean(onStreamEvent && !(context.tools || []).length);
    const call = beginProviderCall(
      context,
      useStreamingAdapter ? 'memory_agent_streaming_provider_adapter' : 'memory_agent_provider_adapter'
    );
    const stream = new AssistantMessageEventStream();
    const partial = {
      role: 'assistant', content: [], api: model.api, provider: model.provider, model: model.id,
    ...
    void (async () => {
      try {
        ...
        const adapterResult = await (useStreamingAdapter ? streamProviderText : generateText)({
          ...
          cache,
          ...(useStreamingAdapter ? { onDelta } : {})
        });
        providerResult = adapterResult;
        if (useStreamingAdapter) {
          const finalText = String(adapterResult.text || '');
          if (!textBlock && finalText) onDelta(finalText);
          if (textBlock) {
            textBlock.text = finalText || textBlock.text;
            stream.push({ type: 'text_end', contentIndex, content: textBlock.text, partial });
          }
          partial.provider = adapterResult.provider || partial.provider;
          partial.model = adapterResult.model || partial.model;
          partial.responseId = adapterResult.responseId || undefined;
          partial.usage = usageFromProvider(adapterResult.usage);
        ...
        emitProviderResult(stream, partial, adapterResult);
        finishProviderCall(call, 'success', partial);
        providerResult = adapterResult;
      } catch (error) {
  ...
  const streamFn = (_model, context, options = {}) => {
    ...
    if (config.textProvider !== 'mock' && config.textProvider !== 'ark' && onStreamEvent) {
      return nativeProviderStream(context, options);
    }
  ...
  const unsubscribe = agent.subscribe((event) => {
    ...
    if (event.type === 'turn_start') {
      modelTurn += 1;
      notifyStream(onStreamEvent, { type: 'turn_start', turn: modelTurn });
    }
    if (event.type === 'message_update' && event.assistantMessageEvent?.type === 'text_delta') {
      notifyStream(onStreamEvent, {
        type: 'delta',
        turn: modelTurn,
  ...
  return {
    ...
    runtime: {
      ...
      stoppedByLimit,
      toolExecution: config.agentLoop.toolExecution,
      toolBoundary: 'server-scoped read-only memory tools plus unlocked capabilities; Pi selects exposed tools'
    }
  };
}
...
export async function runAgentModel(options) {
  if (config.agentRuntime === 'legacy') {
    const { onStreamEvent, ...providerOptions } = options;
    notifyStream(onStreamEvent, { type: 'turn_start', turn: 1 });
    const result = await generateText(providerOptions);
    notifyStream(onStreamEvent, { type: 'delta', turn: 1, delta: String(result.text || '') });
    return {
      ...result,
      runtime: {
...
export function getAgentRuntimeStatus() {
  return {
    ...
    toolLoopEnabled: config.agentRuntime === 'pi',
    toolExposure: 'server_scoped_memory_tools_plus_dynamic_unlocked_props',
