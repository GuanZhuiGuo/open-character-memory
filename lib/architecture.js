...
const sourceSpecs = [
  ...
  { id: 'hybrid_retrieval', file: 'lib/memory.js', label: '两阶段召回、LLM 规划与图谱证据展开', start: 'export function getRetrievalProfile', end: 'export function formatRetrievedMemory' },
...
Pi Agent Core 采用 ReAct 式模型-工具循环：服务端只把当前作用域已解锁、管理员启用且通过白名单的 Skill/MCP 工具暴露给主模型；模型可选择 tool call，能力网关执行后以 toolResult 回传，再由模型继续推理。不得把“已解锁”说成“已执行成功”，也不得把 AGENT_TOOL_EXECUTION 的串行/并行误称为 Plan-Execute。当前没有独立 Planner/Executor 状态机、子 Agent 树或 Deep Agent 工作区。
