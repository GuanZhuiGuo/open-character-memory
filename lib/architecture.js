...
const sourceSpecs = [
  ...
  { id: 'pi_runtime', file: 'lib/pi-runtime.js', label: 'Pi Agent Core 状态、生命周期与 Provider bridge' },
...
function isArchitectureQuestion(question) {
  return /(memory\s*agent|本系统|当前系统|系统架构|代码|实现|逻辑|记忆|事件|轮次|抽取|召回|向量|embedding|prompt|trace|触发器|接口|数据库|sqlite|neo4j|双时态|bitemporal|valid_at|known_at|pi\s*runtime|图谱|上下文|agent|亲密度|并发|压缩|阈值|命名空间|隔离)/i.test(question)
...
Pi Agent Core 负责主回复 turn 的状态与生命周期事件；当前 Provider bridge 只产出文本，现有触发器与道具仍在服务端管线执行，不能声称主模型已通过 Pi 自动调用 Skill 或 MCP。
