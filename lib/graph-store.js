...
async function ensureConstraints() {
  ...
  try {
    for (const statement of [
      ...
      'CREATE CONSTRAINT memory_view_key IF NOT EXISTS FOR (n:MemoryView) REQUIRE n.graph_key IS UNIQUE'
    ]) {
...
function projectionSnapshot(scope) {
  ...
  const views = db.prepare(`SELECT view.* FROM memory_views view
    JOIN events ev ON ev.id = view.canonical_id
    WHERE view.user_id = ? AND view.agent_id = ? AND view.story_id = ? AND view.branch_id = ?
      AND view.status = 'active' AND ev.status = 'active' AND ev.transaction_to = ''`)
    .all(...params).map((row) => ({
      graph_key: graphKey(scopeKey, row.id),
      memory_id: row.id,
      scope_key: scopeKey,
      canonical_id: row.canonical_id,
      canonical_graph_key: graphKey(scopeKey, row.canonical_id),
  ...
  const associations = db.prepare(`SELECT association.* FROM memory_associations association
    ...
    .all(...params).map((row) => ({
      association_key: row.id,
      source_view_graph_key: graphKey(scopeKey, row.source_view_id),
      target_view_graph_key: graphKey(scopeKey, row.target_view_id),
      source_view_id: row.source_view_id,
      target_view_id: row.target_view_id,
      source_event_id: row.source_event_id,
      target_event_id: row.target_event_id,
      source_view_type: row.source_view_type,
      target_view_type: row.target_view_type,
  ...
  return { scopeKey, scope, entities, events, claims, views, associations, eventLinks, claimLinks, edges };
}
...
async function replaceScopeProjection(snapshot) {
  ...
  try {
    await session.executeWrite(async (tx) => {
      ...
      await tx.run(`MATCH (scope:MemoryScope {scope_key: $scopeKey})-[:HAS_ENTITY|HAS_EVENT|HAS_CLAIM|HAS_VIEW]->(node)
      ...
      await tx.run(`MATCH (scope:MemoryScope {scope_key: $scopeKey})
        UNWIND $rows AS row
        MERGE (node:MemoryView {graph_key: row.graph_key}) SET node += row
        MERGE (scope)-[:HAS_VIEW]->(node)
        WITH node, row
        MATCH (event:MemoryEvent {graph_key: row.canonical_graph_key})
        MERGE (node)-[:VIEW_OF]->(event)`, { scopeKey: snapshot.scopeKey, rows: snapshot.views });
      await tx.run(`UNWIND $rows AS row
        MATCH (source:MemoryView {graph_key: row.source_view_graph_key})
        MATCH (target:MemoryView {graph_key: row.target_view_graph_key})
        MERGE (source)-[association:ASSOCIATED_WITH {association_key: row.association_key}]->(target)
        SET association += row`, { rows: snapshot.associations });
      await tx.run(`UNWIND $rows AS row
...
export async function readGraphProjection(scope) {
  ...
  const session = createDriver().session({ database: config.neo4j.database });
  try {
    const [entityResult, eventLinkResult, edgeResult, associationResult] = await session.executeRead(async (tx) => {
      ...
      const associations = await tx.run(`MATCH (:MemoryScope {scope_key: $scopeKey})-[:HAS_VIEW]->(source:MemoryView)
        MATCH (source)-[association:ASSOCIATED_WITH]-(target:MemoryView)
        WHERE source.graph_key < target.graph_key
        RETURN association`, { scopeKey });
      return [entities, eventLinks, edges, associations];
    });
    ...
    return {
      ...
      associationRows: associationResult.records.map((record) => {
        const properties = record.get('association').properties;
        return {
          id: properties.association_key,
          source_view_id: properties.source_view_id,
          target_view_id: properties.target_view_id,
          source_event_id: properties.source_event_id,
          target_event_id: properties.target_event_id,
          source_view_type: properties.source_view_type,
          target_view_type: properties.target_view_type,
          score: Number(properties.score || 0),
          method: properties.method || 'gmm_adaptive',
