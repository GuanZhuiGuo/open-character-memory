...
export function migrate() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS retrieval_profiles (
      ...
      corrective_planner_enabled INTEGER NOT NULL DEFAULT 1,
      planner_max_follow_up_queries INTEGER NOT NULL DEFAULT 2,
      agent_memory_tools_enabled INTEGER NOT NULL DEFAULT 1,
      agent_memory_max_calls INTEGER NOT NULL DEFAULT 2,
      agent_memory_max_queries INTEGER NOT NULL DEFAULT 3,
      graph_hops INTEGER NOT NULL DEFAULT 1,
      granularity_enabled INTEGER NOT NULL DEFAULT 1,
      granularity_router_mode TEXT NOT NULL DEFAULT 'shadow',
      granularity_ab_percent INTEGER NOT NULL DEFAULT 50,
      granularity_score_weight REAL NOT NULL DEFAULT 0.35,
      association_enabled INTEGER NOT NULL DEFAULT 1,
      association_min_similarity REAL NOT NULL DEFAULT 0.50,
      association_max_degree INTEGER NOT NULL DEFAULT 6,
      ppr_enabled INTEGER NOT NULL DEFAULT 1,
      ppr_damping REAL NOT NULL DEFAULT 0.85,
      ppr_iterations INTEGER NOT NULL DEFAULT 12,
      ppr_top_k INTEGER NOT NULL DEFAULT 8,
      ppr_score_weight REAL NOT NULL DEFAULT 0.15,
      context_filter_mode TEXT NOT NULL DEFAULT 'shadow',
      context_filter_min_candidates INTEGER NOT NULL DEFAULT 2,
      context_filter_min_keep INTEGER NOT NULL DEFAULT 1,
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL
    );
    ...
    CREATE TABLE IF NOT EXISTS memory_observations (
      ...
      extraction_mode TEXT NOT NULL DEFAULT 'async',
    ...
    CREATE INDEX IF NOT EXISTS idx_memory_observations_scope
      ON memory_observations(scope_key, thread_id, created_at DESC);

    CREATE TABLE IF NOT EXISTS memory_observation_messages (
      id TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS idx_memory_observation_messages_observation
      ON memory_observation_messages(observation_id, ordinal);

    CREATE TABLE IF NOT EXISTS memory_jobs (
      id TEXT PRIMARY KEY,
      observation_id TEXT NOT NULL UNIQUE REFERENCES memory_observations(id) ON DELETE CASCADE,
      job_type TEXT NOT NULL DEFAULT 'extract_observation',
    ...
    CREATE INDEX IF NOT EXISTS idx_memory_operation_receipts_scope
      ON memory_operation_receipts(scope_key, created_at DESC);
    ...
    CREATE TABLE IF NOT EXISTS memory_views (
      ...
      story_id TEXT NOT NULL DEFAULT 'main_story',
      branch_id TEXT NOT NULL DEFAULT 'main',
      canonical_type TEXT NOT NULL DEFAULT 'event',
      ...
      vector_json TEXT NOT NULL,
      embedding_source TEXT NOT NULL DEFAULT 'native_view',
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL,
      UNIQUE(canonical_type, canonical_id, view_type)
    );
    CREATE INDEX IF NOT EXISTS idx_memory_views_scope
      ON memory_views(user_id, agent_id, story_id, branch_id, status, view_type);
    CREATE INDEX IF NOT EXISTS idx_memory_views_canonical
      ON memory_views(canonical_type, canonical_id, status);
    ...
    CREATE TABLE IF NOT EXISTS memory_associations (
      ...
      target_view_type TEXT NOT NULL,
      score REAL NOT NULL,
      method TEXT NOT NULL DEFAULT 'gmm_adaptive',
      embedding_model TEXT NOT NULL DEFAULT '',
      status TEXT NOT NULL DEFAULT 'active',
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL,
      UNIQUE(source_view_id, target_view_id)
    );
    CREATE INDEX IF NOT EXISTS idx_memory_associations_scope
      ON memory_associations(user_id, agent_id, story_id, branch_id, status, score);
    CREATE INDEX IF NOT EXISTS idx_memory_associations_source
      ON memory_associations(source_view_id, status);
    CREATE INDEX IF NOT EXISTS idx_memory_associations_target
      ON memory_associations(target_view_id, status);
  ...
  for (const table of ['memory_observations', 'memory_operation_receipts']) {
    const columns = db.prepare(`PRAGMA table_info(${table})`).all();
    if (!columns.some((column) => column.name === 'request_hash')) {
      db.exec(`ALTER TABLE ${table} ADD COLUMN request_hash TEXT NOT NULL DEFAULT ''`);
    }
  }
  ...
  for (const [id, sceneId, name] of retrievalProfiles) {
    db.prepare(`INSERT OR IGNORE INTO retrieval_profiles
      ...
      VALUES (?, ?, ?, 1, 1, 1, 1, ?, ?, 4, 20, 20, 0.70, 0.16, 0.10, 0.04, 30,
        ?, ?, 1, 12, 1, 2, 1, 2, 3, 1, 1, 'shadow', 50, 0.35,
