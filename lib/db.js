...
export function migrate() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS event_extraction_profiles (
      ...
      extraction_interval_turns INTEGER NOT NULL DEFAULT 1,
    ...
    CREATE TABLE IF NOT EXISTS retrieval_profiles (
      ...
      min_similarity REAL NOT NULL DEFAULT 0.42,
      ...
      catalog_min_similarity REAL NOT NULL DEFAULT 0.28,
      vector_only_min_similarity REAL NOT NULL DEFAULT 0.42,
  ...
  if (!edgeColumns.some((column) => column.name === 'event_id')) {
    db.exec("ALTER TABLE entity_edges ADD COLUMN event_id TEXT NOT NULL DEFAULT ''");
  }
...
export function seedDefaults() {
  ...
  for (const profile of extractionProfiles) {
    db.prepare(`INSERT OR IGNORE INTO event_extraction_profiles
      ...
      VALUES (?, ?, ?, 1, 'blocking_after_assistant', 'after_every_x_assistant_messages_persisted', 1, ?, 1, 3, 0.35, ?, ?, ?, ?, ?)`)
  ...
  for (const [id, sceneId, name] of retrievalProfiles) {
    db.prepare(`INSERT OR IGNORE INTO retrieval_profiles
      ...
      VALUES (?, ?, ?, 1, 1, 1, 1, 0.42, 0.08, 4, 20, 20, 0.70, 0.16, 0.10, 0.04, 30,
        0.28, 0.42, 1, 12, 1, ?, ?)`)
      .run(id, sceneId, name, timestamp, timestamp);
  }
  db.prepare(`UPDATE retrieval_profiles SET min_similarity = 0.42, event_top_k = 4,
      catalog_min_similarity = 0.28, vector_only_min_similarity = 0.42,
      planner_enabled = 1, planner_max_candidates = 12, graph_hops = 1, updated_at = ?
    WHERE id IN ('retrieval_companion', 'retrieval_education') AND min_similarity <= 0.12`).run(timestamp);
