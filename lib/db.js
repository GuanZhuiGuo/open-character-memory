...
function insertSchema(item, index) {
  ...
  db.prepare(`
    ...
    (id, key, label, category, scenario_type, value_type, scope, description, default_json,
     constraints_json, prompt_template, pinned, required, extraction_mode, sensitivity,
     enabled, sort_order, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    item.id || uid('schema'), item.key, item.label, item.category, item.scenarioType || 'all',
    item.valueType, item.scope, item.description || '', json(item.defaultValue),
    json(item.constraints || {}), item.promptTemplate || '', item.pinned ? 1 : 0,
