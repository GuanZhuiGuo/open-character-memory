...
const DEFAULT_ENTITY_INSTRUCTION = '只抽取被当轮事件实际涉及、后续需要识别或关联的人物、地点、物品、组织、事件与概念；使用稳定规范名，不要把完整句子当成实体。';
...
function buildExtractionPrompt({
  profile, schemas, currentDate, currentClaims, recentDialogue, userText, assistantText, agentName
}) {
...
本次只写入当轮新增、更正或被明确承接的记忆；历史对话只用于指代消歧和判断用户是否承接了角色之前的言行，不得重复抽取旧事件。
...
4. 用户当轮要求角色做某事且角色已完成，或用户当轮明确提及/接受角色之前的言行时，shared_story 可标为 confirmed。角色单方新生成但用户尚未承接的具体承诺或物品只能标为 provisional，不得写成用户事实。
5. 结构化字段只能使用给定 key。explicit_only 仅在用户明确要求时更新；evidence_required 必须有当轮证据；manual 和 derived 不得由模型写入。
6. 亲密度和信任度只能通过 relationship_deltas 更新，单轮变化范围 -5 到 5；普通闲聊应为 0。
...
9. 每轮最多输出 ${profile.max_events_per_turn} 个事件；新建事件 importance 低于 ${profile.min_importance} 时放入 ignored_noise。普通打趣、笑声和没有后续连续性的动作不是记忆。
...
最近 ${profile.context_turns} 轮历史对话（不含当轮，仅消歧）：${JSON.stringify(recentDialogue)}
用户本轮（JSON 字符串）：${JSON.stringify(userText)}
助手本轮（仅理解上下文）：${JSON.stringify(profile.include_assistant ? assistantText : '（本场景未注入助手回复）')}`;
}
...
export function getExtractionPromptContract(sceneId) {
  ...
  const prompt = buildExtractionPrompt({
    ...
    recentDialogue: [{ runtime_placeholder: `previous_${profile.context_turns}_turns_excluding_current_turn` }],
    userText: '{{current_user_message}}',
    assistantText: '{{current_assistant_message}}',
  ...
  return {
    ...
    runtime: {
      ...
      currentTurnIncludedSeparately: true,
...
export async function extractAndCommitMemory({
  ...
  const recentDialogue = extractionDialogue(
    conversationId, profile.context_turns, profile.include_assistant, sourceMessageId
  );
  const extractionPrompt = buildExtractionPrompt({
    profile, schemas, currentDate, currentClaims, recentDialogue, userText, assistantText, agentName: agent.name
  });
