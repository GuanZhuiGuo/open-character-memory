...
export async function storeEvent(event, scope, sourceMessageId) {
  ...
  db.prepare(`INSERT INTO events
    (id, user_id, agent_id, story_id, branch_id, scene_id, event_key, event_type, title, summary,
     status, story_time, sequence_no, source_message_id, importance, metadata_json, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
  ...
  for (const claim of event.claims || []) {
    const result = applyClaim(claim, scope, sourceMessageId, id);
...
function buildExtractionPrompt({
  profile, schemas, currentDate, currentClaims, recentDialogue, userText, assistantText
}) {
...
本次只写入“当轮用户新增、更正或明确确认”的记忆；历史对话只用于指代消歧，不得重复抽取旧事件。
