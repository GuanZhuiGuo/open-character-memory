...
function applyClaim(claim, scope, sourceMessageId, eventId = '', carryEventId = '') {
  ...
  if (active && ['update', 'supersede', 'retract'].includes(operation)) {
    ...
    db.prepare(`UPDATE claims SET status = ?, transaction_to = ?, updated_at = ? WHERE id = ?`)
      .run(operation === 'retract' ? 'retracted' : 'superseded', closing.transactionTo, timestamp, active.id);
    if (operation === 'update') {
      carryClaimVersion(active, claim.valid_from || timestamp, timestamp, carryEventId);
    }
  ...
  const id = uid('claim');
  const temporal = temporalVersion({
    valid_from: claim.valid_from || (operation === 'supersede' ? active?.valid_from : ''),
...
export async function storeEvent(event, scope, sourceMessageId, { projectGraph = true } = {}) {
  ...
  const temporal = temporalVersion({
    valid_from: event.valid_from || (event.operation === 'supersede' ? activeEvent?.valid_from : ''),
  ...
  db.prepare(`INSERT INTO events
    ...
    .run(id, scope.userId, scope.agentId, scope.storyId, scope.branchId, event.scene_id || '', eventKey,
      ...
      sourceSpeaker, json(event.metadata || {}), activeEvent ? Number(activeEvent.version || 1) + 1 : 1,
  ...
  for (const relation of event.relations || []) {
    ...
    const activeEdge = db.prepare(`SELECT * FROM entity_edges
      WHERE user_id = ? AND agent_id = ? AND story_id = ? AND branch_id = ?
        AND source_entity_id = ? AND predicate = ? AND status = 'active' AND transaction_to = ''
      ORDER BY version DESC LIMIT 1`)
      .get(scope.userId, scope.agentId, scope.storyId, scope.branchId, source.id, predicate);
    if (['update', 'supersede', 'retract'].includes(relation.operation)) {
      ...
      db.prepare(`UPDATE entity_edges SET status = ?, transaction_to = ?, updated_at = ?
        WHERE user_id = ? AND agent_id = ? AND story_id = ? AND branch_id = ?
          AND source_entity_id = ? AND predicate = ? AND status = 'active' AND transaction_to = ''`)
        .run(relation.operation === 'retract' ? 'retracted' : 'superseded', closing.transactionTo, timestamp,
          scope.userId, scope.agentId, scope.storyId, scope.branchId, source.id, predicate);
      if (relation.operation === 'update') {
        carryEdgeVersion(activeEdge, relation.valid_from || timestamp, timestamp, carryEventId);
      }
      if (relation.operation === 'retract') continue;
    }
    const edgeId = uid('edge');
    const relationTemporal = temporalVersion({
      valid_from: relation.valid_from || (relation.operation === 'supersede' ? activeEdge?.valid_from : ''),
    ...
    db.prepare(`INSERT INTO entity_edges
      ...
      .run(edgeId, scope.userId, scope.agentId, scope.storyId, scope.branchId, source.id, predicate,
        target.id, json(relation.properties || {}), id, sourceMessageId,
        activeEdge ? Number(activeEdge.version || 1) + 1 : 1, activeEdge?.id || null,
...
export function reconcileExtractedEvent(rawEvent, userText, scope) {
  ...
  const modelRequestedUpdate = ['update', 'supersede', 'retract'].includes(event.operation);
  const relatedMatches = explicitUpdateSignal
    ? findRelatedActiveEvents(event, scope, { allowSingleFallback: true })
    : (modelRequestedUpdate ? findRelatedActiveEvents(event, scope) : []);
...
2. 更新信号 ${JSON.stringify(updateSignals)} 表示同一事件槽位更新，operation 必须是 supersede、update 或 retract。event_key 只能在同一件事的版本之间复用；不同会议、不同人物经历、不同偏好或不同剧情节点必须使用不同 event_key。
...
11. valid_from / valid_to 表示这条事实在故事世界中成立的时间区间，仅在对话有明确时间证据时填写 ISO 时间；不知道就留空。transaction 时间由系统写入，模型不得生成。
12. update 表示世界状态从某时刻真的变化；supersede 表示系统后来发现旧认知有误并纠正；retract 表示撤回且当前不再采用。三者不得混用。
13. 事件抽取指令：${profile.event_instruction}
14. 实体抽取指令：${profile.entity_instruction}
15. 关系与声明抽取指令：${profile.relation_instruction}
${profile.custom_rules ? `16. 场景附加规则：${profile.custom_rules}` : ''}
...
输出格式：
{
  ...
  "events": [{
    ...
    "relations":[{"source":{"name":"...","type":"..."},"predicate":"...","target":{"name":"...","type":"..."},"operation":"create|update|supersede|retract","valid_from":"","valid_to":"","properties":{}}],
    "claims":[{"subject":{"name":"...","type":"..."},"predicate":"...","object":"...","fact_scope":"world_fact|character_belief|rumor|hypothetical|dream|ooc_instruction","modality":"asserted|uncertain|denied","source_speaker":"user|assistant|both","operation":"create|update|supersede|retract","valid_from":"","valid_to":"","confidence":0.0}]
  }],
  "ignored_noise": ["..."]
}
  ...
  for (const rawEvent of (extracted.events || []).slice(0, maxEventsForBatch)) {
    ...
    const evidenceText = sourceMessage?.role === 'user'
      ...
      : (sourceIndex > 0
        ...
        : '');
    const event = reconcileExtractedEvent(rawEvent, evidenceText, scope);
