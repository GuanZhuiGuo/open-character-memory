...
export const EXTRACTION_INTERVAL_MAX_TURNS = 20;
...
export function getEventExtractionProfile(sceneId) {
  ...
  return {
    ...
    extraction_interval_turns: Math.max(1, Number(row.extraction_interval_turns || 1)),
...
本次只写入下方“待抽取对话批次”中新增、更正或被明确承接的记忆；批次之前的历史只用于指代消歧和判断用户是否承接了角色之前的言行，不得重复抽取旧事件。
...
待抽取批次之前最近 ${profile.context_turns} 轮历史对话（仅消歧，不得写入）：${JSON.stringify(recentDialogue)}
...
export function getExtractionPromptContract(sceneId) {
  ...
  const intervalTurns = Math.max(1, Number(profile.extraction_interval_turns || 1));
  ...
  return {
    ...
    runtime: {
      ...
      nextTurnBarrier: '仅达到抽取阈值的轮次阻塞等待'
    },
    objects: [
